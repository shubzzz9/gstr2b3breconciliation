        ...
        <div className="bg-card rounded-b-xl p-5 md:p-8 shadow-lg border border-border border-t-0">
          ...
          {step === 4 && (
            <div>
              ...
              {/* Reconciliation Output Preview */}
              {recoRows && recoRows.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-sm font-bold text-primary mb-2">📊 Reconciliation Output Preview <span className="text-xs font-normal text-muted-foreground">(first 10 rows)</span></h3>
                  <div className="relative w-full overflow-auto border border-border rounded max-h-[300px]">
                    <table className="w-full border-collapse text-[11px]">
                      <thead className="sticky top-0">
                        <tr>
                          {['DATA', 'GSTIN', 'Invoice No', 'Invoice Date', 'Taxable', 'IGST', 'CGST', 'SGST', 'Remarks'].map(h => (
                            <th key={h} className="bg-secondary p-1.5 border border-border font-semibold text-left whitespace-nowrap">{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {recoRows.slice(0, 10).map((row: any, i: number) => {
                          const isMatched = (row['Remarks'] || '').startsWith('Matched');
                          return (
                            <tr key={i} className={isMatched ? 'bg-success/10' : 'bg-destructive/10'}>
                              <td className="p-1.5 border border-border whitespace-nowrap font-medium">{row['DATA'] || ''}</td>
                              <td className="p-1.5 border border-border whitespace-nowrap">{row['GSTIN of supplier'] || row['gstin'] || ''}</td>
                              <td className="p-1.5 border border-border whitespace-nowrap">{row['Invoice number'] || row['invoiceNum'] || ''}</td>
                              <td className="p-1.5 border border-border whitespace-nowrap">{row['Invoice Date'] || row['invoiceDate'] || ''}</td>
                              <td className="p-1.5 border border-border text-right">{(parseFloat(row['Taxable Value (₹)'] || row['taxable'] || 0)).toFixed(2)}</td>
                              <td className="p-1.5 border border-border text-right">{(parseFloat(row['IGST (₹)'] || row['igst'] || 0)).toFixed(2)}</td>
                              <td className="p-1.5 border border-border text-right">{(parseFloat(row['CGST (₹)'] || row['cgst'] || 0)).toFixed(2)}</td>
                              <td className="p-1.5 border border-border text-right">{(parseFloat(row['SGST (₹)'] || row['sgst'] || 0)).toFixed(2)}</td>
                              <td className={`p-1.5 border border-border whitespace-nowrap text-xs font-semibold ${isMatched ? 'text-success' : 'text-destructive'}`}>{row['Remarks'] || ''}</td>
                            </tr>
                          );
                        })}
                      </tbody>
                  ...
                  {recoRows.length > 10 && <p className="text-[10px] text-muted-foreground mt-1">...and {recoRows.length - 10} more rows</p>}
